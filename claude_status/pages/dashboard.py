...
class DashboardPage(common.Page):
    ...
    def _build_charts(self) -> None:
        ...
        self._accounts_card = self._chart_card("账号对比", self._account_bars)
    ...
    def _update_models(
        ...
        self._models_card.set_subtitle(
            f"按{metric.label}，共 {len(groups)} 个模型"
        )
    ...
    def _update_accounts(
        ...
        groups.sort(key=lambda g: metric.of(g.totals), reverse=True)
        labels = [self._state.account_name(g.key) for g in groups[:8]]
        ...
        self._account_bars.set_data(
            labels, [charts.Series(metric.label, values)]
        )
        _leave_label_room(self._account_bars, values)
        self._accounts_card.set_subtitle(f"按{metric.label}排序")
    ...
    def _banner_spec(self) -> BannerSpec | None:
        ...
        if state.dataset.records and state.linked_account() is None:
            return (
                "unlinked",
                "本机日志尚未关联任何账号，用量显示为“未归属”。",
                "link_off",
                [("去关联", "accounts")],
            )
        return None
